...
OUTPUT_TRANSFORM_FILE = "_recovered_tmp.csv"

dag = DAG('recovered_dag', description='ETL for recovered table cases',
          default_args={
              'owner': 'elda.calderon',
...
def insert_process(**kwargs):
    ...
    with db_connection.begin() as transaction:
        transaction.execute("DELETE FROM test.recovered_table WHERE 1=1")
        df.to_sql("recovered_table", con = transaction, schema = "test", if_exists= "replace", index = False)
