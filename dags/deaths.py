...
OUTPUT_TRANSFORM_FILE = "_deaths_tmp.csv"

dag = DAG('deaths_dag', description='ETL for deaths table cases',
          default_args={
              'owner': 'elda.calderon',
...
def insert_process(**kwargs):
    ...
    with db_connection.begin() as transaction:
        transaction.execute("DELETE FROM test.deaths_table WHERE 1=1")
        df.to_sql("deaths_table", con = transaction, schema = "test", if_exists= "replace", index = False)
